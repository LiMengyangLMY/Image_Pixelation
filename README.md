...
* **🎨 专属颜色数据库**：资产隔离已延伸至颜色库，每位用户拥有独立的专属颜色库 (`user_{id}/user_colors.db`)，支持自主增删改查色号数据，与系统默认库完全隔离。
* **⏳ 智能生命周期管理**：
...
* **动态色库选择**：支持在转换前选择使用“系统默认库”或“我的专属库”作为降色基准。
...
* **参考图工具**：内置可缩放、拖拽的参考图片悬浮窗。支持在参考图片上右键直接吸取颜色，并自动调用 API 匹配色库中最接近的已有色号。
* **动态调色盘**：支持通过侧边栏下拉框实时切换使用的颜色数据库，并重新加载相应的面板。
...
* **`get_color_db_path(db_name)`**: 色库路由函数。带有空库自动修复与安全兜底机制，实现用户颜色数据的物理隔离。
...
该数据库集中管理系统的用户账户、VIP 激活码以及验证码，保存在 `data/users.db` 路径下。
...
| **email** | TEXT | 用户注册与找回密码绑定的邮箱地址。 |
| **password_hash** | TEXT | 经过安全哈希加密的密码。 |
| **user_level** | TEXT | 用户等级，标识为 `'common'` (普通用户) 或 `'vip'` (高级会员)，决定了图纸的存储寿命和撤销步数。 |
| **vip_expire_at** | DATETIME | VIP 权限的过期时间（超出该时间将自动降级为 common）。 |

#### 表名: `vip_codes` (VIP 激活码表)

用于管理发行的激活卡密状态。
| 字段名 | 类型 | 说明 |
| :--- | :--- | :--- |
| **code** | TEXT | 16位 VIP 激活卡密字符串（主键/唯一）。 |
| **valid_days** | INTEGER | 该卡密可兑换的 VIP 有效天数。 |
| **is_used** | INTEGER | 状态标识（0: 未使用, 1: 已核销）。 |
| **used_by** | TEXT | 记录核销使用了该卡密的 `username`。 |

#### 表名: `email_verify` (邮箱验证码表)

用于临时存储注册及找回密码时下发的 6 位验证码。
| 字段名 | 类型 | 说明 |
| :--- | :--- | :--- |
| **email** | TEXT | 接收验证码的邮箱地址。 |
| **code** | TEXT | 下发的 6 位数字验证码。 |
| **expire_at** | DATETIME | 验证码的过期时间（通常为下发后 5 分钟），校验时通过比对当前时间验证是否过期。 |

---

### 2. 颜色库数据库 (`colors.db` 及用户专属库)

用于存储拼豆色号、RGB 色值以及用于 Pro 版精准降色算法的 CIELAB 色彩空间坐标。系统默认库位于 `data/Color/colors.db`，用户的专属库位于 `data/Color/user_{id}/` 目录下。

#### 表名: `colors` (色卡表)

| 字段名 | 类型 | 说明 |
| --- | --- | --- |
| **num** | TEXT | 拼豆的色号编码（如 "H2", "501" 等），作为主键 (PRIMARY KEY)。 |
| **R** | INTEGER | 红色通道值 (0-255)。 |
| **G** | INTEGER | 绿色通道值 (0-255)。 |
| **B** | INTEGER | 蓝色通道值 (0-255)。 |
| **lab_l** | REAL | CIELAB 色彩空间的 L* (明度) 坐标。 |
| **lab_a** | REAL | CIELAB 色彩空间的 a* (红绿色度) 坐标。 |
| **lab_b** | REAL | CIELAB 色彩空间的 b* (黄蓝色度) 坐标。 |

---

### 3. 图纸工作台数据库 (`{drawing_id}.db`)

图纸源数据不是集中在一个库里，而是每个图纸一个单独的 SQLite 数据库文件。存储路径为 `data/DrawingData/user_{id}/{drawing_id}.db`（未登录游客为 `public_temp/` 目录）。
每个独立的图纸数据库内部包含两张表：
...
| 字段名 | 类型 | 说明 |
| :--- | :--- | :--- |
| **r** | INTEGER | 像素所在的行号 (Row)。 |
| **c** | INTEGER | 像素所在的列号 (Column)。 |
| **color_id** | TEXT | 该坐标对应的颜色色号，与 `colors` 表的 `num` 对应。 |

#### 表名: `metadata` (图纸元数据表)

用于存储图纸的全局参数及统计信息，采用 Key-Value 键值对结构。
| 字段名 | 类型 | 说明 |
| :--- | :--- | :--- |
| **key** | TEXT | 键名，主键 (PRIMARY KEY)。 |
| **value** | TEXT | 存放以 JSON 格式序列化的字符串数据。 |

*注：`metadata` 表中通常包含以下两个核心 Key：*

* `'dimensions'`：图纸的宽和高，如 `"[40, 40]"`。
* `'color_code_count'`：当前图纸的颜色使用统计字典（JSON 字符串），包含每个使用到的色号出现的次数 (count) 及对应的 RGB/Lab 数值，便于快速生成统计图例和查找表。
...
**Q4: 切换专属颜色库后，为什么有些图纸的颜色变黑了？**
如果您使用的是“专属颜色库”，而图纸元数据中记录的色号在您的专属库中找不到对应数据时，底层渲染引擎会触发安全兜底策略将其渲染为黑色（RGB: 0,0,0）。请在颜色管理页确认您的专属库包含了所有基础图纸色号。
